...
void BaseDMD::end()const {
}

#else // Use timers

// Forward declarations for tracking currently running DMDs
static void register_running_dmd(const BaseDMD *dmd);
static bool unregister_running_dmd(const BaseDMD *dmd);
...
*/
static volatile const BaseDMD **running_dmds = 0;
...
static void register_running_dmd(const BaseDMD *dmd)
{
  ...
  if(empty == -1) { // Grow array to fit new entry
    ...
    running_dmds = (volatile const BaseDMD **)resized;
  }
