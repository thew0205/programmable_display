...
void DMDFrameDoubleBuffer::selectFont(const uint8_t* font)
{
  this->font = (uint8_t *)font;
}

int DMDFrameDoubleBuffer::drawChar(const int x, const int y, const char letter, DMDGraphicsMode mode, const uint8_t *font)const
{
  if (!font)
    font = this->font;
...
template <class StrType> __attribute__((always_inline)) inline void _drawString(const DMDFrameDoubleBuffer *dmd, int x, int y, StrType str, DMDGraphicsMode mode, const uint8_t *font)
{
  struct FontHeader header;
...
template <class StrType> __attribute__((always_inline)) inline unsigned int _stringWidth(const DMDFrameDoubleBuffer *dmd, const uint8_t *font, StrType str)
{
...
void DMDFrameDoubleBuffer::drawString_P(int x, int y, const char *flashStr, DMDGraphicsMode mode, const uint8_t *font)const
{
  if (!font)
    font = this->font;
  if (x >= (int)width || y >= height)
...
unsigned int DMDFrameDoubleBuffer::stringWidth_P(const char *flashStr, const uint8_t *font)const
{
  if (!font)
    font = this->font;
  _FlashStringWrapper wrapper(flashStr);
...
void DMDFrameDoubleBuffer::drawString(int x, int y, const char *bChars, DMDGraphicsMode mode, const uint8_t *font)const
{
  if (!font)
    font = this->font;
  if (x >= (int)width || y >= height)
...
void DMDFrameDoubleBuffer::drawString(int x, int y, const String &str, DMDGraphicsMode mode, const uint8_t *font)const
{
  if (!font)
    font = this->font;
  if (x >= (int)width || y >= height)
...
int DMDFrameDoubleBuffer::charWidth(const char letter, const uint8_t *font)const
{
  struct FontHeader header;
...
unsigned int DMDFrameDoubleBuffer::stringWidth(const char *bChars, const uint8_t *font)const
{
  if (!font)
    font = this->font;
...
unsigned int DMDFrameDoubleBuffer::stringWidth(const String &str, const uint8_t *font)const
{
  if (!font)
    font = this->font;
