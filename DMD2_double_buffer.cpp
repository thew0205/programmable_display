...
void SPIDMD::writeSPIData(volatile uint8_t *rows[4], const int rowsize)const
{
...
void BaseDMD::scanDisplay()
{
  ...
  volatile uint8_t *rows[4] = { // Scanning out 4 interleaved rows
    first_bitmap + (scan_row + 0) * rowsize,
    first_bitmap + (scan_row + 4) * rowsize,
    first_bitmap + (scan_row + 8) * rowsize,
    first_bitmap + (scan_row + 12) * rowsize,
  };
...
void SoftDMD::writeSPIData(volatile uint8_t *rows[4], const int rowsize)const
{
...
BaseDMD::BaseDMD(byte panelsWide, byte panelsHigh, byte pin_noe, byte pin_a,
                 byte pin_b, byte pin_sck)
  : DMDFrameDoubleBuffer(panelsWide * PANEL_WIDTH, panelsHigh * PANEL_HEIGHT),
    scan_row(0), pin_noe(pin_noe), pin_a(pin_a), pin_b(pin_b),
    pin_sck(pin_sck),
#ifdef ESP8266
